...
# drop edge randomly from the whole KG graph
def drop_edge_random(item2entities, p_drop, padding, config):
    res = dict()
    for item, es in item2entities.items():
        ...
        res[item] = torch.IntTensor(new_es).to(config.device)
    return res
...
# drop the whole edge_index
class AugKGCL(nn.Module):
    def __init__(self, gcn_model, config, tau):
        super(AugKGCL, self).__init__()
        self.gcn_model : ModelKGCL = gcn_model
        self.tau = tau
        self.config = config
    ...
    def get_kg_views(self):
        ...
        view1 = drop_edge_random(kg, self.config.kg_p_drop, self.gcn_model.num_entities, self.config)
        view2 = drop_edge_random(kg, self.config.kg_p_drop, self.gcn_model.num_entities, self.config)
        return view1, view2
    ...
    def get_ui_views_pgrace(self):
        ...
        g = torch.sparse.FloatTensor(index, data, torch.Size(coo.shape)).coalesce().to(self.config.device)
    ...
    def get_ui_views_weighted(self, item_stabilities, stab_weight):
        ...
        deg_col = torch.FloatTensor(item_degrees).to(self.config.device)
        ...
        weights = (1-self.config.ui_p_drop)/torch.mean(stab_weight*kg_weights)*(stab_weight*kg_weights)
    ...
    def ui_drop_weighted(self, item_mask):
        ...
        item_np = self.gcn_model.dataset.trainItem
        keep_idx = list()
        if self.config.uicontrast=="WEIGHTED-MIX":
            # overall sample rate = 0.4*0.9 = 0.36
            for i, j in enumerate(item_np.tolist()):
                if item_mask[j] and random()>0.6:
                    keep_idx.append(i)
            # add random samples
            interaction_random_sample = sample(list(range(len(item_np))), int(len(item_np)*self.config.mix_ratio))
    ...
    def print_kg_dict(self, kg):
        sizes = 0
        for k, v in kg.items():
            sizes = sizes + len(v) # v is a tensor 
        print(sizes)
    ...
    def get_views(self, aug_side="both"):
        ...
        if aug_side=="ui" or not self.config.kgc_enable:
            kgv1, kgv2 = None, None
        else:
            kgv1, kgv2 = self.get_kg_views()
            

        if aug_side=="kg" or self.config.uicontrast=="NO" or self.config.uicontrast=="ITEM-BI":
            uiv1, uiv2 = None, None
        else:
            if self.config.uicontrast=="WEIGHTED" or self.config.uicontrast=="WEIGHTED-MIX":
                # [item_num]
                stability = self.item_kg_stability(kgv1, kgv2).to(self.config.device)
                # print(stability.cpu().tolist())
                # item drop -> 2 views
                uiv1 = self.get_ui_views_weighted(stability, 1)
                # uiv2 = self.ui_drop_random(self.config.ui_p_drop)
                uiv2 = self.get_ui_views_weighted(stability, 1)
            elif self.config.uicontrast=="PGRACE":
                uiv1 = self.get_ui_views_pgrace()
                uiv2 = self.get_ui_views_pgrace()
            elif self.config.uicontrast=="RANDOM":
                uiv1 = self.ui_drop_random(self.config.ui_p_drop)
                uiv2 = self.ui_drop_random(self.config.ui_p_drop)
